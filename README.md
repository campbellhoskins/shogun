...
Shogun is an AI pipeline that turns unstructured travel duty-of-care policy documents into structured ontology graphs. It extracts typed entities, relationships, and source anchors, then saves the result as a reloadable ontology.
...
| **3 — Merge** | Deduplicates entities (deterministic + LLM semantic) and remaps edges | Unified ontology graph |
| **4 — Relationships** | Full-document relationship extraction over all deduplicated entities | Additional edges |

When it finishes, the pipeline saves all outputs to `results/runs/{timestamp}_{policy_name}/` and exits.
...
### 3. Query with the Reasoning Agent (BETA)
...
```
src/
├── main.py            # Entry point: document → pipeline → save to runs
├── pipeline.py        # Orchestrates all 5 stages, calls save_run()
...
├── merge.py           # Stage 3: entity deduplication + relationship remapping
├── relationships.py   # Stage 4: full-document relationship extraction
├── cross_section.py   # Standalone cross-section relationship extraction (not in pipeline)
