...
def parse_policy(policy_text: str, client: Anthropic | None = None) -> OntologyGraph:
    """Parse a policy document into an OntologyGraph using Claude."""
    if client is None:
        client = Anthropic()
    ...
    response = client.messages.create(
        ...
        messages=[
            {
                "role": "user",
                "content": f"Extract the complete ontology graph from this corporate travel duty of care policy:\n\n{policy_text}",
            }
        ],
    )
